...
/** UX-2.5 / UX-2.7 — Panel shell API. Shell only — no Header/Body creation. */
...
export function Panel({
  ...
  return (
    <section
      ...
      aria-label={title}
      style={style}
      className={`flex shrink-0 flex-col min-h-0 overflow-hidden border border-[var(--app-border)] bg-[var(--app-surface)] ${responsiveHide}`}
    >
      {children}
    </section>
  );

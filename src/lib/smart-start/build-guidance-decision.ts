...
function decision(partial: GuidanceDecision): GuidanceDecision {
  return {
    ...partial,
...
function importThenAnalyze(interpretation: string): GuidanceDecision {
  return decision({
    interpretation,
    explanation:
...
function analyzeAvailable(interpretation: string): GuidanceDecision {
  return decision({
    interpretation,
    explanation:
...
function importFirstForAnalyze(interpretation: string): GuidanceDecision {
  return decision({
    interpretation,
    explanation:
...
function askDataSource(interpretation: string): GuidanceDecision {
  return decision({
    interpretation,
    explanation:
...
function guidanceForWinner(
  ...
  if (winnerId === "analyze-dataset") {
    const subsequent: SmartStartCardOptionId[] = ["analyze-dataset", "analyze-workspace"];
    return decision({
      interpretation: "Quiere incorporar datos (por ejemplo un CSV) para poder analizarlos.",
      explanation:
  ...
  // analyze-workspace
  if (status === "loaded") {
    return analyzeAvailable("Quiere analizar los datos de la sesión.");
  }
  if (status === "empty") {
    return importFirstForAnalyze("Quiere analizar datos y la sesión está vacía.");
  }
  return askDataSource("Quiere analizar, pero no está claro si ya hay datos cargados.");
}
...
export function buildGuidanceDecision(
  ...
  if (text.trim().length === 0) {
    return decision({
      ...
      explanation: "Escriba qué desea hacer. Las tarjetas de abajo son las entradas visibles.",
      prerequisite: null,
      suggestedCardIds: [],
      primaryCardId: null,
      clarification: null,
  ...
  if (
    ...
    if (isLoadedFollowUp(text) || (status === "loaded" && !isImportFollowUp(text))) {
      return analyzeAvailable("Indicó que los datos ya están disponibles.");
    }
    if (isImportFollowUp(text) || status === "empty") {
      return importThenAnalyze("Indicó que necesita incorporar un archivo.");
    }
    return decision({
  ...
  if (isExploreRequest(text)) {
    ...
    if (status === "empty") {
      return decision({
        ...
        suggestedCardIds: ["analyze-dataset", "analyze-workspace", "compare-datasets"],
        primaryCardId: "analyze-dataset",
        clarification: null,
        uncertainty: "none",
        candidateIntentIds: candidates,
    ...
    return askDataSource("Quiere explorar qué puede hacer, pero no está claro si hay datos.");
  }
  ...
  return guidanceForWinner(winner.intentId, status, [
    winner.intentId,
    ...candidates.filter((id) => id !== winner.intentId),
  ]);
}

...
const D5_VGB_PCA_SPEC: VisualGraphSpecification = {
  ...DEFAULT_VISUAL_GRAPH_SPECIFICATION,
  graphType: "pca",
...
const writeGoldenFixture = (
  ...
  if (!serialized.ok) {
    throw new Error(
      `Failed to serialize golden pca fixture ${targetName}: ${JSON.stringify(serialized.errors)}`
    );
  }
  ...
  if (
    ...
    throw new Error(`Golden pca fixture ${targetName} violates VGB-R1.`);
  }
...
const generatePcaGoldenFixture = () => {
  ...
  writeGoldenFixture("project-v2-dataset5-with-pca.sgproj", merged);
};

generatePcaGoldenFixture();

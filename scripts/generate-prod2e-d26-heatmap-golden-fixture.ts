...
const D5_VGB_HEATMAP_SPEC: VisualGraphSpecification = {
  ...DEFAULT_VISUAL_GRAPH_SPECIFICATION,
  graphType: "heatmap",
  xVariable: null,
  yVariable: null,
...
const writeGoldenFixture = (
  ...
  if (!serialized.ok) {
    throw new Error(
      `Failed to serialize golden heatmap fixture ${targetName}: ${JSON.stringify(serialized.errors)}`
    );
  }
...
const generateHeatmapGoldenFixture = () => {
  ...
  writeGoldenFixture("project-v2-dataset5-with-heatmap.sgproj", merged);
};

generateHeatmapGoldenFixture();

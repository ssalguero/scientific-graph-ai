...
const GOLDEN_EXPORTED_AT = "2026-07-10T12:00:00.000Z";
...
const D5_VGB_JOURNAL_SCATTER_SPEC: VisualGraphSpecification = {
  ...DEFAULT_VISUAL_GRAPH_SPECIFICATION,
  graphType: "scatter",
  xVariable: "x",
...
const writeGoldenFixture = (
  ...
  if (!serialized.ok) {
    throw new Error(
      `Failed to serialize golden publication-preset fixture ${targetName}: ${JSON.stringify(serialized.errors)}`
    );
  }
...
const generatePublicationPresetGoldenFixture = () => {
  ...
  writeGoldenFixture("project-v2-dataset5-with-publication-preset.sgproj", merged);
};

generatePublicationPresetGoldenFixture();

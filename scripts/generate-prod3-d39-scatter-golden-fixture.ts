...
const D5_VGB_SCATTER_PRO_SPEC: VisualGraphSpecification = {
  ...DEFAULT_VISUAL_GRAPH_SPECIFICATION,
  graphType: "scatter",
  xVariable: "x",
...
const writeGoldenFixture = (
  ...
  if (!serialized.ok) {
    throw new Error(
      `Failed to serialize golden scatter fixture ${targetName}: ${JSON.stringify(serialized.errors)}`
    );
  }
...
const generateScatterProGoldenFixture = () => {
  ...
  writeGoldenFixture("project-v2-dataset5-with-scatter-pro.sgproj", merged);
};

generateScatterProGoldenFixture();
